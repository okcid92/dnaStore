...
export default function Commander() {
  ...
  const generateWhatsAppMessage = (order, cart, formData) => {
    ...
    message += `🚚 *Mode:* ${
      formData.delivery_mode === "livraison" ? "Livraison" : "Retrait"
    }\n`;
    message += `💰 *Paiement:* ${
      formData.payment_method === "cash" ? "À la livraison" : "Mobile Money"
    }\n\n`;

    message += `📦 *Articles:*\n`;
    cart.forEach((item) => {
      message += `• ${item.name} ${item.size ? `(Taille ${item.size})` : ""} x${
        item.quantity
    ...
    return (
      <Layout>
        <div className="container mx-auto px-6 py-24 text-center">
          ...
          <h1 className="text-2xl font-display font-bold text-brand-light mb-4">
            Votre panier est vide
          </h1>
          <button
            onClick={() => router.push("/produits")}
            className="liquid-button px-8 py-3 rounded-sm text-xs uppercase tracking-widest"
          >
            Voir les produits
          </button>
        </div>
          ...
          <div className="liquid-glass rounded-sm p-6 md:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, delivery_mode: "livraison" })
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, delivery_mode: "retrait" })
                    }
                    className={`py-3 px-4 rounded-sm border-2 transition-all ${
              ...
              <p className="text-xs text-brand-muted text-center">
                En cliquant, votre commande sera enregistrée et vous serez
                redirigé vers WhatsApp pour confirmation.
              </p>
            </form>
          </div>
          ...
          <div className="liquid-glass rounded-sm p-6 md:p-8 h-fit">
            ...
            <div className="space-y-4 mb-6">
              {cart.map((item, index) => (
                <div
                  key={index}
                  className="flex gap-4 pb-4 border-b border-brand-border"
                >
                  <img
                  ...
                  <div className="flex-1">
                    <h3 className="text-sm font-bold text-brand-light">
                      {item.name}
                    </h3>
                    {item.size && (
