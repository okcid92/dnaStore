    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-32 text-center">
          ...
          <h1 className="text-3xl font-display font-bold text-brand-light mb-4 tracking-widest">Produit non trouvé</h1>
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 md:px-6 py-8 md:py-16">
        <button onClick={() => router.back()} className="mb-8 text-brand-blue hover:text-brand-blue/80 text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div className="liquid-glass rounded-sm p-6 md:p-8">
            <div className="mb-6">
              <h1 className="text-3xl md:text-4xl font-display font-bold text-brand-light mb-3 tracking-wider">{product.name}</h1>
              {product.description && (
                <p className="text-brand-muted text-sm leading-relaxed">{product.description}</p>
              )}
            </div>

            <div className="flex items-baseline gap-3 mb-8 pb-6 border-b border-brand-border">
              <span className="text-4xl font-display font-bold text-brand-light">{product.price.toLocaleString()}</span>
              <span className="text-brand-muted text-sm uppercase tracking-widest">FCFA</span>
            </div>

            {product.sizes && product.sizes.length > 0 && (
              <div className="mb-6">
                <label className="block text-xs font-bold text-brand-light uppercase tracking-widest mb-3">Taille</label>
            ...
            {product.colors && product.colors.length > 0 && (
              <div className="mb-6">
                <label className="block text-xs font-bold text-brand-light uppercase tracking-widest mb-3">Couleur</label>
                <div className="flex gap-2 flex-wrap">
            ...
            <div className="mb-8">
              <label className="block text-xs font-bold text-brand-light uppercase tracking-widest mb-3">Quantité</label>
              <div className="flex items-center gap-4">
                ...
                <span className="text-2xl font-display font-bold text-brand-light min-w-[3rem] text-center">{quantity}</span>
                <button
      ...
      {relatedProducts.length > 0 && (
        <section className="py-24 border-t border-brand-border mt-16">
          <div className="container mx-auto px-4 md:px-6">
            <h2 className="text-2xl md:text-3xl font-display font-light text-brand-light mb-12 text-center">
